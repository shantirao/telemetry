...
case 2:
   if (Date) sprintf(Date,"%02.2d/%02.2d/%04.4d",  //world dmy
#ifdef XP_WIN
   ...
   if (Time) sprintf(Time,"%02.2d:%02.2d:%02.2d",
#ifdef XP_WIN
           tb.wHour,tb.wMinute,tb.wSecond);
#else
           tb->tm_hour,tb->tm_min,tb->tm_sec);
           ...
           break;
case 1:
   if (Date) sprintf(Date,"%02.2d/%02.2d/%04.4d", //US mdy
#ifdef XP_WIN
...
default:
   if (Date) sprintf(Date,"%04.4d%02.2d%02.2d",
#ifdef XP_WIN
           tb.wYear,tb.wMonth,tb.wDay);
#else
           tb->tm_year+1900,tb->tm_mon+1,tb->tm_mday);
